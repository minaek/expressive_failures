# Modified, from pezzottone: ~sibi/sandbox/john_rapprentice/rapprentice/scripts
import numpy as np, os.path as osp
from numpy import inf, zeros, dot, r_, pi
from numpy.linalg import norm, inv
from threading import Thread

from lfd.rapprentice import \
        retiming, math_utils as mu,conversions as conv, func_utils, resampling
...
VEL_RATIO = 1
ACC_RATIO = .3
DOF_VELOCITY_LIMITS = np.array([
          100.   ,  100.   ,  100.   ,  100.   ,  100.   ,  100.   ,
          100.   ,  100.   ,  100.   ,  100.   ,  100.   ,  100.   ,
          0.013,    6.   ,    5.   ,    2.088,    2.082,    3.27 ,
          3.3  ,    3.6  ,    3.078,    3.6  ,    0.5  ,    0.5  ,
          0.5  ,    0.5  ,    0.2  ,   10.   ,    2.088,    2.082,
          3.27 ,    3.3  ,    3.6  ,    3.078,    3.6  ,    0.5  ,
          0.5  ,    0.5  ,    0.5  ,    0.2  ])
DOF_ACCELERATION_LIMITS = np.array([
          100.   ,    100.   ,    100.   ,    100.   ,    100.   ,
          100.   ,    100.   ,    100.   ,    100.   ,    100.   ,
          100.   ,    100.   ,  10000.   ,      2.645,     15.   ,
           30.   ,     30.   ,     30.   ,     30.   ,     30.   ,
           10.   ,     10.   ,   1000.   ,   1000.   ,   1000.   ,
         1000.   ,   1000.   ,      0.65 ,     30.   ,     30.   ,
           30.   ,     30.   ,     30.   ,     10.   ,     10.   ,
...
class PR2(object):
    ...
    def __init__(self, env=None):
        ...
        self.joint_listener = TopicListener("/pr2capabilities/joint_states", sm.JointState)

        # openrave to gazebo conversions
        joint_msg = self.get_last_joint_message()        
        self.joint_names_gazebo = joint_msg.name
        inds_gazebo2rave = np.array([self.robot.GetJointIndex(name) for name in self.joint_names_gazebo])
        if 'l_gripper_joint' in self.joint_names_gazebo:
            inds_gazebo2rave[self.joint_names_gazebo.index('l_gripper_joint')] = \
                    self.robot.GetJointIndex('l_gripper_l_finger_joint')
        if 'r_gripper_joint' in self.joint_names_gazebo:
            inds_gazebo2rave[self.joint_names_gazebo.index('r_gripper_joint')] = \
                    self.robot.GetJointIndex('r_gripper_l_finger_joint')

        self.good_gazebo_inds = np.flatnonzero(inds_gazebo2rave != -1) # ros joints inds with matching rave joint
        self.rave_inds = inds_gazebo2rave[self.good_gazebo_inds] # openrave indices corresponding to those joints
        self._set_rave_limits_to_soft_joint_limits()
        self.update_rave()                                                      
    ...
    def update_rave(self):
        ros_values = list(self.get_last_joint_message().position)
        # Adjust gripper angle values (open gripper in ROS / Gazebo is 0.08, but is 0.54 in OpenRave)
        if 'l_gripper_joint' in self.joint_names_gazebo:
            ros_values[self.joint_names_gazebo.index('l_gripper_joint')] *= 6
        if 'r_gripper_joint' in self.joint_names_gazebo:
            ros_values[self.joint_names_gazebo.index('r_gripper_joint')] *= 6

        rave_values = [ros_values[i_ros] for i_ros in self.good_gazebo_inds]
...
class TrajectoryControllerWrapper(object):
    ...
    def __init__(self, pr2, controller_name):
        ...
        self.joint_names = rospy.get_param("/pr2capabilities/%s/joints"%controller_name)

        self.n_joints = len(self.joint_names)

        msg = self.pr2.get_last_joint_message()
        self.gazebo_joint_inds = [msg.name.index(name) for name in self.joint_names]

        self.controller_pub = rospy.Publisher("pr2capabilities/%s/command"%controller_name, tm.JointTrajectory)        
        self.vel_limits = [DOF_VELOCITY_LIMITS[i_gazebo] * VEL_RATIO for i_gazebo in self.gazebo_joint_inds]
        self.acc_limits = [DOF_ACCELERATION_LIMITS[i_gazebo] * ACC_RATIO for i_gazebo in self.gazebo_joint_inds]

    def get_joint_positions(self):
        msg = self.pr2.get_last_joint_message()
        return np.array([msg.position[i_gazebo] for i_gazebo in self.gazebo_joint_inds])
    ...
    def follow_joint_trajectory(self, traj):
        ...
        print 'follow_joint_trajectory', self.vel_limits
...
class Arm(TrajectoryControllerWrapper):
    ...
    L_POSTURES = dict(        
        ...
        low_side = [  2,  0.3,   1.5,  -2,   0  ,  0,  1.9],
        low_side2 = [  2,  0.3,   1.5,  -2,   1.7  ,  0,  0.1],
        low_side3 = [  2,  0.3,   1.5,  -2,   1.7  , 0,  3.074],
        low_side4 = [  2.5,  0.7,   1.5,  -2,   1.7  , 0,  3.5],
        low_side5 = [  2.5,  -0.5,   1.5,  -2,   1.7  , 0,  2.7]
    )    
...
class Torso(TrajectoryControllerWrapper):
    def __init__(self,pr2):
        ...
        self.pr2 = pr2
        self.controller_name = "torso_controller"
        self.joint_names = rospy.get_param("/pr2capabilities/%s/joints"%self.controller_name)
        self.n_joints = len(self.joint_names)

        msg = self.pr2.get_last_joint_message()
        self.gazebo_joint_inds = [msg.name.index(name) for name in self.joint_names]

        self.controller_pub = rospy.Publisher("/pr2capabilities/%s/command"%self.controller_name, stdm.Float64)
        self.vel_limits = [0.2]
        self.acc_limits = [inf]

        #self.torso_client = actionlib.SimpleActionClient('pr2capabilities/torso_controller/position_joint_action', pcm.SingleJointPositionAction)
...
class Gripper(object):
    ...
    def __init__(self,pr2,lr):
        ...
        self.controller_pub = rospy.Publisher("/pr2capabilities/%s/command"%self.controller_name, tm.JointTrajectory)        
    ...
    def open(self, max_effort=default_max_effort):
        self.set_angle(.09, max_effort = max_effort)
    ...
    def get_angle(self):
        return self.pr2.joint_listener.last_msg.position[self.gazebo_joint_inds[0]]

    def get_velocity(self):
        return self.pr2.joint_listener.last_msg.velocity[self.gazebo_joint_inds[0]]

    def get_effort(self):
        return self.pr2.joint_listener.last_msg.effort[self.gazebo_joint_inds[0]]
