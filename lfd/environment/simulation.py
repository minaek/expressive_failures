...
import trajoptpy
import bulletsimpy
from lfd.rapprentice import animate_traj, retiming
import numpy as np
from lfd.environment.robot_world import RobotWorld
from lfd.environment import sim_util, settings
import importlib
...
class StaticSimulation(object):
    ...
    def add_objects(self, objs_to_add, consider_finger_collisions=True):
        ...
        #if consider_finger_collisions:
        if not consider_finger_collisions:
...
class DynamicSimulation(StaticSimulation):
    ...
    def add_objects(self, sim_objs, consider_finger_collisions=False):
        ...
        self._include_gripper_finger_collisions()
        # add static objects
        super(DynamicSimulation, self).add_objects(static_sim_objs, consider_finger_collisions=consider_finger_collisions)
        # add dynamic objects
        self._remove_bullet()
...
class DynamicSimulationRobotWorld(DynamicSimulation, RobotWorld):
    ...
    #             traj[:,ind] = np.maximum(traj[:,ind], get_binary_gripper_angle(False))
        ...
        transition_traj = retiming.retime_traj(self.robot, dof_inds, transition_traj,
        ...
        traj = retiming.retime_traj(self.robot, dof_inds, traj, max_cart_vel=max_cart_vel)  # make the trajectory slow enough for the simulation
